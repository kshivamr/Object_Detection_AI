...
if __name__ == "__main__":
    ...
    last_servo_time = 0
    cooldown = 5  # seconds between servo actions

    while True:
        img = picam2.capture_array()
        img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)  # ✅ Fix: convert 4 channels → 3 channels

        result, objectInfo = getObjects(img, 0.45, 0.2, objects=['cup', 'horse'])

        # If object detected, move servo (with cooldown)
        if objectInfo and (time.time() - last_servo_time > cooldown):
            print("Target detected! Activating servo...")
            servo.angle = -90
            time.sleep(1)
            servo.angle = 90
            time.sleep(1)
